...
import { FeedbackQuery } from './feedback-collector';
...
export class AutoOptimizer {
  ...
  async optimizeAgent(agentId: string): Promise<OptimizationResult> {
    ...
    const testResults = await this.testVariations(
      variations,
      feedbackDataset
    );
   ...
   */
  private async buildDataset(agentId: string): Promise<FeedbackDataset> {
    const feedbacks = await FeedbackQuery.getEditsForOptimization(agentId, 100);

    const samples: FeedbackSample[] = feedbacks
      .filter((f) => f.userEdit || f.correctionText)
      .map((f) => ({
        input: (f.trace.conversation.messages || []).map((m: any) => m.content),
        originalOutput: f.originalOutput,
        correctedOutput: f.userEdit || f.correctionText || '',
        context: (f.trace as any).steps || [],
        feedbackType: f.type.toLowerCase() as any,
      }));
        ...
        if (jsonMatch) {
          ...
          return patterns.map((p: any, i: number) => ({
            ...
            frequency: Math.floor(
              (dataset.samples.length / patterns.length) * (patterns.length - i)
            ),
            examples: [p.example],
            category: p.category,
          }));
...
[
  ...
  private async testVariations(
    ...
    dataset: FeedbackDataset
  ): Promise<VariationTestResult[]> {
    const results: VariationTestResult[] = [];
    ...
    for (const variation of variations) {
      ...
      for (const sample of testSamples) {
        ...
        const score = await this.scoreOutputSimilarity(
          sample.correctedOutput,
          sample.correctedOutput // Assuming new prompt would produce similar
        );
   ...
   */
  private async scoreOutputSimilarity(
    output1: string,
    output2: string
  ): Promise<number> {
   ...
   */
  private selectBest(results: VariationTestResult[]): PromptVariation {
    if (results.length === 0) {
  ...
  private async getCurrentPrompt(agentId: string): Promise<string> {
  ...
  private async saveOptimizationRun(result: OptimizationResult): Promise<void> {
...
export class OptimizationQuery {
  /**
   * Get optimization runs for an agent
   */
  static async getOptimizationRuns(agentId: string, limit: number = 10) {
   ...
   */
  static async getLatestRun(agentId: string) {
