...
export class ABTestManager {
  ...
  async startABTest(config: ABTestConfig): Promise<string> {
    ...
    console.log(
      `[ABTestManager] Started A/B test ${test.id} for agent ${config.agentId} (${(config.trafficSplit * 100).toFixed(0)}% to B)`
    );
   ...
   */
  async getActiveTest(agentId: string): Promise<ABTestResult | null> {
   ...
   */
  async selectVariant(agentId: string): Promise<'A' | 'B' | null> {
  ...
  async recordTraceResult(
    ...
    if (variant === 'A') {
      ...
      const newScore =
        (currentScore * test.variantATraces + score) / newCount;
      ...
      const newScore =
        (currentScore * test.variantBTraces + score) / newCount;
  ...
  private async checkTestCompletion(testId: string): Promise<void> {
    ...
    console.log(
      `[ABTestManager] Test ${testId} completed. Winner: ${winner} (A: ${scoreA.toFixed(1)}, B: ${scoreB.toFixed(1)})`
    );
   ...
   */
  async selectWinner(testId: string, winner: 'A' | 'B'): Promise<void> {
    const test = await prisma.agentABTest.findUnique({
      where: { id: testId },
    });
   ...
   */
  async getTests(agentId: string, limit: number = 10): Promise<ABTestResult[]> {
    ...
    return tests.map((test) => ({
      id: test.id,
      agentId: test.agentId,
      status: test.status.toLowerCase() as any,
      variantATraces: test.variantATraces,
      variantBTraces: test.variantBTraces,
      variantAScore: test.variantAScore || undefined,
      variantBScore: test.variantBScore || undefined,
      winningVariant: (test.winningVariant as 'A' | 'B') || undefined,
      startedAt: test.startedAt,
      endedAt: test.endedAt || undefined,
    }));
  }
