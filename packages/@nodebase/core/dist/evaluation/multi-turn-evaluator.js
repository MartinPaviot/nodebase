...
exports.EvaluationQuery = exports.MultiTurnEvaluator = void 0;
const client_1 = require("@prisma/client");
const sentiment_analyzer_1 = require("./sentiment-analyzer");
...
class MultiTurnEvaluator {
    ...
    constructor() {
        this.sentimentAnalyzer = new sentiment_analyzer_1.SentimentAnalyzer();
        this.hallucinationDetector = new hallucination_detector_1.HallucinationDetector();
    }
    /**
     * Evaluate a complete conversation
     */
    async evaluateConversation(conversationId) {
     ...
     */
    async detectGoalCompletion(messages, traces) {
        ...
        if (lastUserMessage) {
            ...
            const hasPositiveSignal = positiveKeywords.some((kw) => content.includes(kw));
            ...
            const hasNegativeSignal = negativeKeywords.some((kw) => content.includes(kw));
        ...
        const totalTools = traces.reduce((sum, t) => sum + t.toolSuccesses + t.toolFailures, 0);
     ...
     */
    async inferSatisfaction(messages, traces) {
        let score = 3; // Default neutral
        // Check for positive sentiment in last user message
        const lastUserMessage = messages.filter((m) => m.role === 'user').pop();
        if (lastUserMessage) {
            const sentiment = await this.sentimentAnalyzer.analyze(lastUserMessage.content);
        ...
        const avgToolFailures = traces.reduce((sum, t) => sum + t.toolFailures, 0) / traces.length;
     ...
     */
    async detectFailures(traces, messages) {
        const failures = [];
        // Check for blocked L3 eval
        if (traces.some((t) => t.l3Blocked)) {
            failures.push('unsafe_output');
        }
        // Check for high tool failure rate
        const totalTools = traces.reduce((sum, t) => sum + t.toolSuccesses + t.toolFailures, 0);
        const failureRate = totalTools > 0
        ...
        const hasHallucination = await this.hallucinationDetector.detect(messages, traces);
        if (hasHallucination) {
     ...
     */
    async suggestImprovements(traces, failures) {
    ...
    async loadTraces(conversationId) {
    ...
    async saveEvaluation(conversationId, result) {
...
exports.MultiTurnEvaluator = MultiTurnEvaluator;
// ============================================================================
...
class EvaluationQuery {
     ...
     */
    static async getEvaluation(conversationId) {
     ...
     */
    static async getAgentEvaluations(agentId, limit = 50) {
    ...
    static async getPerformanceMetrics(agentId, days = 30) {
        ...
        const goalCompletionRate = evals.filter((e) => e.goalCompleted).length / total;
        const avgSatisfaction = evals.reduce((sum, e) => sum + e.userSatisfactionScore, 0) / total;
        // Count failure modes
