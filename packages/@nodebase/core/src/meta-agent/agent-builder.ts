...
export class AgentBuilder {
  ...
  private agentTester: AgentTester;
   ...
   */
  async buildAgent(request: AgentBuildRequest): Promise<BuiltAgent> {
   ...
   */
  private selectTools(intent: AgentIntent, suggestedIntegrations?: string[]): Tool[] {
    const tools: Tool[] = [];
   ...
   */
  private createToolConfig(toolType: string): Tool | null {
   ...
   */
  private getIntegrationTools(integration: string): Tool[] {
   ...
   */
  private async generatePrompt(intent: AgentIntent, tools: Tool[]): Promise<string> {
...
Generate the system prompt. Respond with ONLY the prompt text, no preamble.`;
   ...
   */
  private configureEvalRules(intent: AgentIntent): EvalRules {
    const l1Assertions: L1Assertion[] = [];
    const l2Criteria: L2Criterion[] = [];
    ...
    if (intent.category === 'SALES' || intent.category === 'SUPPORT') {
      l1Assertions.push(
        ...
        { check: 'professional_tone', severity: 'warn' }
      );
    }
    ...
    if (intent.outputFormat === 'email') {
      l1Assertions.push(
        { check: 'has_subject', severity: 'block' },
        { check: 'has_greeting', severity: 'warn' },
        { check: 'has_signature', severity: 'warn' },
    ...
    l2Criteria.push(
      ...
      { name: 'completeness', description: 'Covers all required points', weight: 0.3 },
      { name: 'relevance', description: 'Relevant to the request', weight: 0.2 },
      { name: 'tone', description: 'Appropriate tone and style', weight: 0.1 }
    );
   ...
   */
  private determineActions(intent: AgentIntent): AgentAction[] {
    ...
    if (
      (intent.category === 'SALES' || intent.category === 'SUPPORT') &&
   ...
   */
  private selectModel(intent: AgentIntent): AgentModel {
    const tier = this.intentAnalyzer.getRecommendedModelTier(intent);

    switch (tier) {
      case 'haiku':
        return AgentModel.ANTHROPIC; // Using ANTHROPIC (could specify Haiku variant in future)
      case 'opus':
        return AgentModel.ANTHROPIC; // Using ANTHROPIC
      case 'sonnet':
      default:
        return AgentModel.ANTHROPIC;
    }
  }
   ...
   */
  private getTemperature(intent: AgentIntent): number {
   ...
   */
  private getIntegrations(tools: Tool[]): string[] {
