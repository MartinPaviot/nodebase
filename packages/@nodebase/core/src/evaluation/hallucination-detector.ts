...
export class HallucinationDetector {
   ...
   */
  async detect(messages: any[], traces: TraceData[]): Promise<boolean> {
    ...
    for (const message of assistantMessages) {
      ...
      const hasIndicator = this.hallucinationIndicators.some((indicator) =>
   ...
   */
  private getToolFailureRate(traces: TraceData[]): number {
   ...
   */
  private hasVagueReferences(text: string): boolean {
    const vaguePatterns = [
   ...
   */
  private hasUncertainLanguage(text: string): boolean {
