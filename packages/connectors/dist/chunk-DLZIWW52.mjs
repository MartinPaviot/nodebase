...
var CalendarConnector = class extends BaseConnector {
  ...
  async createEvent(input, context) {
    ...
    const response = await this.apiRequest(
      context,
      "POST",
      `/calendar/v3/calendars/${encodeURIComponent(input.calendarId)}/events?sendUpdates=${input.sendUpdates}`,
      event
    );
    return response;
  }
  async listEvents(input, context) {
    const params = new URLSearchParams({
      ...
      singleEvents: String(input.singleEvents),
    ...
    const response = await this.apiRequest(
      context,
      "GET",
      `/calendar/v3/calendars/${encodeURIComponent(input.calendarId)}/events?${params.toString()}`
    );
    if (!response.success) {
      return response;
    }
  ...
  async updateEvent(input, context) {
    ...
    const response = await this.apiRequest(
      context,
      "PATCH",
      `/calendar/v3/calendars/${encodeURIComponent(input.calendarId)}/events/${input.eventId}`,
      updates
    );
    return response;
  }
  async deleteEvent(input, context) {
    const response = await this.apiRequest(
      context,
      "DELETE",
      `/calendar/v3/calendars/${encodeURIComponent(input.calendarId)}/events/${input.eventId}?sendUpdates=${input.sendUpdates}`
    );
