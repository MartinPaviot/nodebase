...
export default function IntegrationsPage() {
  ...
  function getProviderEndpoint(type: IntegrationType, action: "connect" | "disconnect"): string {
    const config = integrationConfigs.find(c => c.type === type);
    if (!config) return `/api/integrations/google/${action}`;
  ...
  async function handleConnect(type: IntegrationType) {
    setConnecting(type);
    try {
      const endpoint = getProviderEndpoint(type, "connect");
  ...
  async function handleDisconnect(type: IntegrationType) {
    ...
    try {
      const endpoint = getProviderEndpoint(type, "disconnect");
