      ...
      {showSetFieldsOption && (
        ...
        >
          ...
          <span className="text-sm text-gray-700">
            {fieldsMode === "manual" ? "Set all fields to Auto" : "Set all fields to Manual"}
          </span>
        </button>
      )}
      <button
...
function SelectActionNode({ id, data, selected }: { id: string; data: { label?: string; hasWarning?: boolean; fieldsMode?: "manual" | "auto" } & NodeCallbacks; selected?: boolean }) {
  const [showOptions, setShowOptions] = useState(false);
  const optionsBtnRef = useRef<HTMLButtonElement>(null);
...
// Composio Action Node - displays Composio integration actions
function ComposioActionNode({
  id,
  data,
  selected
}: {
  ...
  return (
    <div className="flex flex-col items-center relative">
      ...
      {/* Add child action button - only when no outgoing edge */}
      {!data.hasOutgoingEdge && (
        <button
          onClick={() => setShowMenu(!showMenu)}
          className="mt-1 flex items-center justify-center size-6 rounded-full bg-cyan-400 hover:bg-cyan-500 text-white transition-colors cursor-pointer shadow-sm hover:shadow"
        >
...
interface NodeCallbacks {
  ...
  onAddErrorHandling?: (nodeId: string) => void;
  onSetFieldsMode?: (nodeId: string, mode: "manual" | "auto") => void;
}
...
export const FlowEditorCanvas = forwardRef<FlowEditorCanvasRef, FlowEditorCanvasProps>(function FlowEditorCanvas({
  ...
  // Store node action handlers in refs for use in node components
  const onRenameNodeRef = useRef<(nodeId: string) => void>(() => {});
      ...
      initialFlowData.nodes.forEach((node) => {
        ...
        const baseNode: Node = {
          ...
          data: {
            ...
            // Add node action callbacks for the options menu
            onRename: (nodeId: string) => onRenameNodeRef.current(nodeId),
            onDelete: (nodeId: string) => onDeleteNodeRef.current(nodeId),
            onAddErrorHandling: (nodeId: string) => onAddErrorHandlingRef.current(nodeId),
            onSetFieldsMode: (nodeId: string, mode: "manual" | "auto") => onSetFieldsModeRef.current(nodeId, mode),
  ...
  // Handle rename node - selects the node and triggers rename mode
  const handleRenameNode = useCallback((nodeId: string) => {
    const node = nodes.find(n => n.id === nodeId);
    if (!node) return;

    // Select the node and enable rename mode
    ...
    // Also notify parent to open the node panel
    onNodeSelectRef.current?.(nodeId, node.type || null, { ...node.data, isRenaming: true });
  }, [nodes, setNodes]);
  ...
  // Handle add error handling - adds an "On error" node connected to the specified node
  const handleAddErrorHandling = useCallback((nodeId: string) => {
    const sourceNode = nodes.find(n => n.id === nodeId);
    if (!sourceNode) return;

    // Create a new error handler node
    const errorNodeId = `error-${Date.now()}`;
    const newErrorNode: Node = {
      id: errorNodeId,
      type: "chatOutcome",
      position: {
      ...
      data: {
        ...
        hasOutgoingEdge: false,
        onSelectAction: (actionId: string, sourceNodeId?: string) => handleSelectActionRef.current(actionId, sourceNodeId),
        onOpenReplaceModal: (nId: string) => onOpenReplaceModalRef.current?.(nId),
        onRename: (nId: string) => onRenameNodeRef.current(nId),
        onDelete: (nId: string) => onDeleteNodeRef.current(nId),
        onAddErrorHandling: (nId: string) => onAddErrorHandlingRef.current(nId),
        onSetFieldsMode: (nId: string, mode: "manual" | "auto") => onSetFieldsModeRef.current(nId, mode),
      },
    };

    // Add the error node
    setNodes((nds) => [...nds, newErrorNode]);

    // Create an edge from the source node to the error node with a special "error" handle
    const newEdge: Edge = {
      id: `e-${nodeId}-${errorNodeId}-error`,
      source: nodeId,
      sourceHandle: "error",
      target: errorNodeId,
      type: "addButton",
      data: { onInsertNode: handleInsertNodeRef.current },
      style: { ...edgeStyle, stroke: "#EF4444" }, // Red color for error edge
    };
  ...
  // Handle set fields mode - toggles between manual and auto mode for all fields
  const handleSetFieldsMode = useCallback((nodeId: string, mode: "manual" | "auto") => {
    setNodes((nds) =>
      nds.map((node) => {
        if (node.id === nodeId) {
          return {
  ...
  onRenameNodeRef.current = handleRenameNode;
  onDeleteNodeRef.current = handleDeleteNode;
  onAddErrorHandlingRef.current = handleAddErrorHandling;
  onSetFieldsModeRef.current = handleSetFieldsMode;
  ...
  return (
    <>
      ...
      <AddActionModal
        ...
        agentId={agentId}
        onSelectStructuralNode={(actionId) => {
